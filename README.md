...
4. Agent 调用工具生成岗位匹配摘要和 Top gaps。
...
- JD 匹配摘要：展示命中关键词、缺失关键词、Top gaps、JD 证据和建议补强位置。
...
- `generate_job_match_summary`：生成岗位匹配摘要。
- `analyze_jd_top_gaps`：返回本轮最值得优先处理的 2-3 个岗位缺口。
...
当前 JD 匹配是轻量关键词和证据链分析，不是完整语义匹配模型。

它会：

- 从 JD 中提取中英文关键词。
- 排除 JD 字段后，在简历正文中判断命中和缺失。
- 根据 JD 出现次数、出现位置，以及“必须、要求、负责、核心、优先、加分”等提示词给缺口排序。
- 为每个 Top gap 生成 JD 原文证据、简历可补位置、优先级原因、建议补强方向和风险类型。

风险类型包括：

- `can_improve`：简历已有相邻经历，可直接补强表达。
- `needs_user_confirmation`：需要用户确认是否有真实经历。
- `insufficient_evidence`：简历暂无可支撑证据，不能编造。
