...
test('completed 面试报告展示行动报告结构', async ({ page }) => {
  ...
  await expect(page.getByText('AI 面试报告')).toBeVisible()
  await expect(page.getByText('报告概览')).toBeVisible()
  await expect(page.getByText('面试结论')).toBeVisible()
  await expect(page.getByText('综合得分')).toBeVisible()
  await expect(page.getByText('题目数量').first()).toBeVisible()
  ...
  await expect(page.getByText('项目方向相关，但负责边界和量化结果没有证明清楚。').first()).toBeVisible()
