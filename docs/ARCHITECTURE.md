...
- `backend/app/runtime/`：Agent harness、权限确认、运行时事件转换和超时 session 清理。
- `backend/app/state/`：`AgentSessionStore` 和会话事件日志，支撑确认恢复与 SSE cursor 回放。
- `backend/app/tools/resume/`：简历 Agent 可调用的业务工具。
- `backend/app/services/`：业务服务、外部 API 集成、领域逻辑和供应商适配。
...
`backend/app/main.py` 的鉴权中间件会在进入受保护 API 前统一校验 Bearer token 或 HttpOnly access-token Cookie。`/api/billing/paypal/webhook` 和 `/api/resumes/download` 是显式豁免路径。
...
- 前端路由保护由 `frontend/src/proxy.ts` 处理，受保护页面包括 `/dashboard`、`/settings`、`/interviews`、`/resume` 和 `/resumes`。
...
3. `AgentHarness.create_resume_session()` 在 `AgentSessionStore` 中创建 session，随后 `run_resume_stream()` 驱动 `ResumeAgent` 和 `backend/app/tools/resume/` 工具。
