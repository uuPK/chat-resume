...
class VolcEngineASRService:
    ...
    async def create_realtime_session(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not all([self.access_key, self.secret_key, self.app_id]):
                raise ValueError(
                    "火山引擎配置不完整，请检查 VOLCENGINE_ACCESS_KEY、VOLCENGINE_SECRET_KEY、VOLCENGINE_ASR_APP_ID 环境变量"
                )
            ...
            try:
                websocket = await asyncio.wait_for(
                    websockets.connect(
                        websocket_url, ping_interval=None, ping_timeout=None
                    ),
                    timeout=10.0,
                )
    ...
    async def send_audio_chunk(
        self, websocket, audio_data: bytes, sequence: int
    ) -> None:
        """发送音频数据块"""
        try:
            # 将音频数据编码为base64
            ...
            message = {"signal": "audio", "audio": audio_base64, "sequence": sequence}
    ...
    async def end_recognition(self, websocket, sequence: int) -> Dict[str, Any]:
        """结束语音识别"""
        try:
            end_message = {"signal": "end", "sequence": sequence}
        ...
        try:
            ...
            async for audio_chunk in audio_stream:
                ...
                try:
                    ...
                    yield {"success": False, "error": str(e), "sequence": sequence}
