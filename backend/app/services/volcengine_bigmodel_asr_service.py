...
class VolcEngineBigModelASRService:
    ...
    def __init__(self):
        ...
        self.resource_id = getattr(
            settings, "VOLCENGINE_ASR_RESOURCE_ID", "volc.bigasr.sauc.duration"
        )
    ...
    def _create_message(
        ...
        header = self._create_header(
            message_type, message_flags, serialization, compression
        )
    ...
    async def _read_message(self, websocket) -> Dict[str, Any]:
        ...
        try:
            ...
            # 读取序列号（如果存在）
            sequence = None
            if header["message_flags"] in [
                self.MSG_FLAG_SEQUENCE_POSITIVE,
                self.MSG_FLAG_SEQUENCE_NEGATIVE,
            ]:
    ...
    def _create_audio_request(
        self, audio_data: bytes, sequence: int, is_last: bool = False
    ) -> bytes:
    ...
    async def create_websocket_connection(
        ...
        if not all([self.app_key, self.access_token]):
            raise ValueError(
                "火山引擎配置不完整，请检查 VOLCENGINE_APP_KEY、VOLCENGINE_ACCESS_TOKEN 环境变量"
            )
        ...
        try:
            websocket = await asyncio.wait_for(
                websockets.connect(endpoint, additional_headers=headers), timeout=10.0
            )
    ...
    async def simple_recognition(
        self, audio_data: bytes, config: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """简单的一次性语音识别"""
        try:
            # 检查配置
            ...
            try:
                ...
                if (
                    result_response["header"]["message_type"]
                    == self.MSG_TYPE_FULL_SERVER_RESPONSE
                ):
            ...
            return {"success": False, "error": str(e), "message": "语音识别失败"}
    ...
    async def streaming_recognition(
        ...
        on_final_result: Optional[Callable[[Dict[str, Any]], None]] = None,
        ...
        try:
            ...
            try:
                ...
                async for audio_chunk in audio_chunks:
                    if audio_chunk:
                        # 发送音频数据
                        audio_request = self._create_audio_request(
                            audio_chunk, sequence
                        )
                        await websocket.send(audio_request)
                        sequence += 1
                        ...
                        try:
                            result_response = await asyncio.wait_for(
                                self._read_message(websocket), timeout=0.1
                            )
                ...
                if (
                    final_response["header"]["message_type"]
                    == self.MSG_TYPE_FULL_SERVER_RESPONSE
                ):
            ...
            yield {"success": False, "error": str(e), "message": "流式语音识别失败"}
