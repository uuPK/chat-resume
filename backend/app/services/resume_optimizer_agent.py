...
# RESUME_OPTIMIZER_PROMPTS 将在后续版本中实现，暂时使用空字典
RESUME_OPTIMIZER_PROMPTS = {
    "GREETING": "你好！我是你的AI职业顾问，让我帮你优化简历。",
    "GENERAL_CONVERSATION": "作为AI职业顾问，我将帮助你优化简历。",
}
...
class ResumeOptimizerAgent:
    ...
    async def stream_response(
        ...
        prompt_template = RESUME_OPTIMIZER_PROMPTS.get(
            state, RESUME_OPTIMIZER_PROMPTS["GENERAL_CONVERSATION"]
        )
    ...
    def _determine_conversation_state(self, history: List[Dict[str, str]]) -> str:
        ...
        for msg in recent_messages:
            if msg.get("role") == "assistant":
                ...
                if any(
                    keyword in content
                    for keyword in ["目标岗位", "求职目标", "应聘职位", "目标职位"]
                ):
        ...
        if any(
            ...
            for keyword in ["工作经历", "工作经验", "职业经历"]
        ):
            return "WORK_EXPERIENCE_FOCUS"
        elif any(
            keyword in last_assistant_msg
            for keyword in ["项目经验", "项目经历", "项目"]
        ):
            return "PROJECT_FOCUS"
        elif any(
            keyword in last_assistant_msg
            for keyword in ["教育背景", "学历", "教育经历"]
        ):
            return "EDUCATION_FOCUS"
        elif any(
            keyword in last_assistant_msg
            for keyword in ["技能", "专业技能", "核心技能"]
        ):
            return "SKILLS_FOCUS"

        # 检查是否在生成具体建议
        if (
            "建议" in last_assistant_msg
            or "优化" in last_assistant_msg
            or "改为" in last_assistant_msg
        ):
    ...
    def _format_resume_for_prompt(self, resume_data: Dict[str, Any]) -> str:
        ...
        if personal_info := resume_data.get("personal_info"):
            ...
            if info_items:
                formatted_sections.append("【个人信息】\n" + "\n".join(info_items))
        ...
        if work_exp := resume_data.get("work_experience"):
            ...
            if work_items:
                formatted_sections.append("【工作经历】\n" + "\n".join(work_items))
        ...
        if projects := resume_data.get("projects"):
            ...
            if project_items:
                formatted_sections.append("【项目经验】\n" + "\n".join(project_items))
        ...
        if education := resume_data.get("education"):
            ...
            if edu_items:
                formatted_sections.append("【教育背景】\n" + "\n".join(edu_items))
    ...
    def extract_suggestions_from_response(
        ...
        matches = re.findall(
            suggestion_pattern, response_content, re.DOTALL | re.IGNORECASE
        )
    ...
    def _infer_section_from_content(self, content: str) -> str:
        ...
        if any(
            ...
            for keyword in ["公司", "职位", "工作", "负责", "主导"]
        ):
            return "work_experience"
        elif any(
            keyword in content_lower for keyword in ["项目", "开发", "设计", "实现"]
        ):
            return "projects"
        elif any(
            keyword in content_lower for keyword in ["大学", "学院", "专业", "学历"]
        ):
            return "education"
