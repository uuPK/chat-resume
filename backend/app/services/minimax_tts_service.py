...
class MiniMaxTTSService:
    ...
    def __init__(self):
        ...
        self.api_base = getattr(
            settings, "MINIMAX_API_BASE", "https://api.minimaxi.com"
        )
    ...
    async def text_to_speech(
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if base_resp.get("status_code") != 0:
                    ...
                    if base_resp.get("status_code") == 1008:
                        ...
                        raise Exception(
                            f"MiniMax API错误 ({base_resp.get('status_code')}): {status_msg}"
                        )
                ...
                return {
                    ...
                    "duration": extra_info.get("audio_length", 0) / 1000
                    if extra_info.get("audio_length")
                    else 0,  # 转换为秒
                    "format": format,
            ...
            raise Exception(
                f"MiniMax TTS API错误: {e.response.status_code} - {error_detail}"
            )
        except Exception as e:
            raise Exception(f"TTS服务调用失败: {str(e)}")

    async def clone_voice(
        self, audio_file_path: str, voice_name: str = "cloned_voice"
    ) -> Dict[str, Any]:
        """
        语音克隆
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                clone_payload = {"file_id": file_id, "voice_name": voice_name}

                clone_response = await client.post(
                    clone_url, json=clone_payload, headers=self.headers
                )
            ...
            error_detail = e.response.text
            raise Exception(
                f"语音克隆API错误: {e.response.status_code} - {error_detail}"
            )
        except Exception as e:
    ...
    async def get_voice_list(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_detail = e.response.text
            raise Exception(
                f"获取音色列表API错误: {e.response.status_code} - {error_detail}"
            )
        except Exception as e:
            raise Exception(f"获取音色列表失败: {str(e)}")

    def get_interviewer_voice_config(
        self, interviewer_type: str = "professional"
    ) -> Dict[str, str]:
        """
        获取面试官音色配置
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await self.text_to_speech(
                text="测试", voice_id="female-tianmei-jingpin", model="speech-02-turbo"
            )
