...
class InterviewReportService:
    ...
    def _safe_json_parse(
        self, content: str, default_value: Dict[str, Any] | None = None
    ) -> Dict[str, Any]:
        """安全的JSON解析函数"""
        try:
            # 清理可能的非JSON内容
    ...
    async def generate_comprehensive_report(
        ...
        if not conversation_history:
            ...
            if interview_session.status == "active":
                ...
                # 提供更详细的错误信息
                questions_data = self._get_json_field(interview_session.questions)
                answers_data = self._get_json_field(interview_session.answers)
                questions_count = len(questions_data or [])
                answers_count = len(answers_data or [])
                raise ValueError(
                    f"面试数据不完整，无法生成报告。问题数量：{questions_count}，答案数量：{answers_count}。请确保面试已正常进行并保存了问答数据。"
                )
        ...
        tasks = [
            ...
            self._analyze_keywords_coverage(
                conversation_history, str(interview_session.jd_content or "")
            ),
        ...
        report = {
            ...
            "interview_mode": self._get_interview_mode_name(
                str(interview_session.interview_mode or "")
            ),
            ...
            "performance_level": self._get_performance_level(
                int(interview_session.overall_score or 0)
            )
            ...
            "answered_questions": len(answers_data or []),
    ...
    def _get_json_field(
        self, field: Union[InstrumentedAttribute, Column, List, None]
    ) -> List:
        """安全获取JSON字段数据，处理Column类型"""
        if field is None:
            return []
        if isinstance(field, (InstrumentedAttribute, Column)):
            # 如果是Column对象，返回空列表（在实际运行时会被正确的数据替换）
            return []
        if isinstance(field, list):
            return field
        # 如果是其他类型，尝试转换为列表
        try:
            return list(field) if hasattr(field, "__iter__") else []
        except Exception:
            return []
    ...
    def _build_conversation_history(
        ...
        questions = self._get_json_field(interview_session.questions)
        answers = self._get_json_field(interview_session.answers)

        # 兼容性处理：如果answers为空但有历史数据，尝试从其他地方获取
        if (
            not answers
            and hasattr(interview_session, "feedback")
            and interview_session.feedback
        ):
            # 老版本可能将对话存储在feedback中
            if (
                isinstance(interview_session.feedback, dict)
                and "conversation" in interview_session.feedback
            ):
        ...
        for i in range(min(len(questions), len(answers))):
            ...
            if question_text and answer_text:
                conversation.append(
                    {"question": question_text, "answer": answer_text, "index": i}
                )

        return conversation

    async def _analyze_competency_scores(
        self, conversation: List[Dict[str, str]], interview_session: InterviewSession
    ) -> Dict[str, int]:
        """分析能力评分"""
        ...
        try:
            messages = [
                {
                    "role": "system",
                    "content": "你是一个专业的面试评估师，能够客观准确地评估候选人的各项能力。",
                },
            ...
            return {
                "job_fit": int(min(100, max(60, overall_score - 5))),
                "technical_depth": int(min(100, max(60, overall_score + 5))),
                "project_exposition": int(min(100, max(60, overall_score - 10))),
                "communication": int(min(100, max(60, overall_score))),
                "behavioral": int(min(100, max(60, overall_score - 8))),
            }
    ...
    async def _generate_ai_feedback(
        ...
        try:
            messages = [
                {
                    "role": "system",
                    "content": "你是一个经验丰富的面试官，能够给出专业而有建设性的面试反馈。",
                },
                {"role": "user", "content": prompt},
            ]
            ...
            feedback = self._safe_json_parse(
                content, {"highlights": [], "improvements": []}
            )
            return {
    ...
    async def _analyze_conversation_details(
        self, conversation: List[Dict[str, str]], interview_session: InterviewSession
    ) -> List[Dict[str, Any]]:
        """分析每个问答的详细反馈"""
        ...
        for item in conversation:
            # 为每个问答生成详细评估
            try:
                evaluation = await self._evaluate_single_qa(
    ...
    async def _evaluate_single_qa(self, question: str, answer: str) -> Dict[str, Any]:
        ...
        return self._safe_json_parse(
            content, {"score": 75, "analysis": "分析暂时不可用"}
        )

    async def _analyze_keywords_coverage(
        self, conversation: List[Dict[str, str]], jd_content: str
    ) -> Dict[str, Any]:
        """分析关键词覆盖率"""
        ...
        for keyword in keywords:
            frequency = len(
                re.findall(rf"\b{re.escape(keyword)}\b", all_answers, re.IGNORECASE)
            )
            mentioned = frequency > 0

            keyword_analysis.append(
                {"keyword": keyword, "mentioned": mentioned, "frequency": frequency}
            )
        ...
        return {"keywords": keyword_analysis, "coverage_rate": round(coverage_rate)}
    ...
    async def _analyze_frequent_words(
        ...
        for i, question_data in enumerate(questions):
            ...
            if question_text:
                all_questions.append(
                    {"question": question_text, "type": question_type, "index": i}
                )

        return all_questions

    async def _generate_reference_answers(
        self, interview_session: InterviewSession
    ) -> List[Dict[str, Any]]:
        """为所有问题生成AI参考答案"""
        questions = self._get_json_field(interview_session.questions)
            ...
            if question_text:
                try:
                    ...
                    reference_answer = await self._generate_single_reference_answer(
                        question_text, interview_session, resume_content
                    )
    ...
    async def _generate_single_reference_answer(
        self,
        question: str,
        ...
        messages = [
            {
                "role": "system",
                "content": "你是一位经验丰富的面试顾问，擅长基于候选人的实际简历背景为面试问题提供专业的参考答案。",
            },
            {"role": "user", "content": prompt},
        ]
